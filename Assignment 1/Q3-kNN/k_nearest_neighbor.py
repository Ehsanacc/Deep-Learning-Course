...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                # Compute the L2 distance between X_test[i] and X_train[j]
    ...
    def compute_distances_no_loops(self, X):
        ...
        """
        # Calculate the squared L2 distances using broadcasting and matrix operations
        # The shape of X is (num_test, D) and self.X_train is (num_train, D)

        # Step 1: Calculate the squared terms for test and train points separately
        test_sq = np.sum(X ** 2, axis=1).reshape(-1, 1)  # Shape (num_test, 1)
        train_sq = np.sum(self.X_train ** 2, axis=1)  # Shape (num_train,)

        # Step 2: Calculate the cross term
        cross_term = np.dot(X, self.X_train.T)  # Shape (num_test, num_train)

        # Step 3: Use the formula (a - b)^2 = a^2 + b^2 - 2ab to get distances
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # Find the k nearest neighbors for the ith test point
            nearest_neighbors = np.argsort(dists[i])[:k]  # Indices of the k nearest neighbors
            closest_y = self.y_train[nearest_neighbors]   # Labels of these neighbors
            ...
            # Determine the most common label (break ties by choosing the smaller label)
